...
def to_rows(cases: List[Dict], test_category: str = "functional", test_type: str = "smoke") -> List[List[str]]:
    ...
    [TestID, Title, Steps, Expected, Priority, Category, Type].
    ...
    for i, c in enumerate(cases, start=1):
        ...
        category = str(test_category).strip().title()
        type_value = str(test_type).strip().title()
        rows.append([tid, title, steps, expected, priority, category, type_value])
    return rows
...
def write_csv(rows: List[List[str]], path: Path) -> None:
    ...
    This simple writer:
    - Writes a header row `["TestID","Title","Steps","Expected","Priority","Category","Type"]`.
    ...
    header = ["TestID", "Title", "Steps", "Expected", "Priority", "Category", "Type"]
    lines = [",".join(header)]
    for r in rows:
